...
export class FireBaseModel {
  ...
  _getFilter = async (filterData) => {
    const taskQuery = query(
      ...
      where(filterData._field, filterData.op, filterData.value)
    );
    let rlt = [],
      tmp;
  ...
  _add = async (id, newData) => {
    let res = {};
    await setDoc(doc(db, this.collectionName, id), { ...newData })
      .then((result) => {
  ...
  _update = async (id, updateData) => {
    let res = {};
    await setDoc(doc(db, this.collectionName, id), { ...updateData })
      .then((result) => {
        res = { state: true, message: "success" };
      })
      .catch((error) => {
